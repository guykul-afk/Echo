...
`);
  ...
  for (const fixture of DANIEL_FIXTURES) {
    ...
    const isNaturalSilence = !bespokeQ?.shouldIntervene || bespokeQ?.strategy === 'no_intervention' || erv < 0.81;
...
| :--- | :---: | :---: | :---: | :--- |
| **מדד ביטחון יתר (Overconfidence Bias)** | \`${earlySign}${earlyCalib.overconfidenceBiasIndex.toFixed(2)}\` | \`${midSign}${midCalib.overconfidenceBiasIndex.toFixed(2)}\` | \`${fullSign}${fullCalib.overconfidenceBiasIndex.toFixed(2)}\` | **התכנסות ריאליסטית: פיכחון של 40% בהערכות יתר** |
| **ציון ברייר (Brier Score)** | \`${earlyCalib.brierScore.toFixed(3)}\` | \`${midCalib.brierScore.toFixed(3)}\` | \`${fullCalib.brierScore.toFixed(3)}\` | **שיפור בדיוק החיזוי לאורך הזמן** |
