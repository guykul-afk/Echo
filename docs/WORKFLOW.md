...
## 5. ניהול תיעוד ארכיטקטוני ותהליכי פיתוח (Engineering Documentation & Architecture)

כדי לאפשר בחינה של התפתחות הקוד לאורך זמן ומציאת שורש לתקלות בקלות, אנו מאמצים את כללי הברזל הבאים בניהול הפיתוח:

### א. ניהול החלטות ארכיטקטוניות (Architecture Decision Records - ADR)
כל החלטה משמעותית הנוגעת לתשתית, בחירת טכנולוגיה, או שינוי לוגי מרכזי, תתועד כקובץ Markdown נפרד וממוספר בתיקיית \docs/adr/\.
* **איך מבצעים?** מעתיקים את \docs/adr/template.md\, ממלאים את ההקשר וההשלכות, ומוסיפים ל-Git.
* **הערך המוסף:** כשיש באג בעתיד, אפשר לחזור למספר ה-ADR ולהבין *למה* הקוד נכתב כך ומה היו האילוצים באותו זמן.

### ב. תבנית הודעות Commit סטנדרטית (Conventional Commits) - מומלץ
כדי לאפשר הפקת CHANGELOG אוטומטי ולהבין מיד מה כל שינוי עושה בקוד, מומלץ (כהנחיה מנחה) לכתוב הודעות Commit לפי המבנה הבא:
* \eat: add semantic router\ - להוספת יכולת חדשה.
* \ix: resolve cognitive load loop in engine\ - לתיקון באג.
* \docs: update architecture diagram\ - לשינוי תיעוד.
* \efactor: restructure agent memory\ - לשינוי קוד שלא מוסיף פיצ'ר או מתקן באג.

**קישוריות (Traceability):** במידה וה-Commit מיישם החלטה ארכיטקטונית, יש לציין את ה-ADR. לדוגמה: \eat: switch to firestore (ADR #0001)\.

### ג. תיעוד חי (Living Documentation)
* דיאגרמות מורכבות (כמו C4, ERD, זרימת נתונים) ינוהלו כקוד טקסטואלי באמצעות [Mermaid](https://mermaid.js.org/) בתוך קבצי ה-Markdown.
* **כלל אצבע:** כאשר הקוד משתנה ומשנה זרימה משמעותית, הדיאגרמה הרלוונטית חייבת להתעדכן *באותו Pull Request / Commit*. אין להשאיר דיאגרמות שמנותקות ממצב הקוד בפועל.
